...
新增的音乐执行器：接收 CreativePlan 中的音乐意图，产出结构化 MIDI JSON，再经确定性工具链落盘为 `music.mid`，并通过 SoundFont/FluidSynth 合成为 `music.wav`（纯音乐）。
...
- 素材收集与校验、压缩/入库
- 本地渲染（可插拔 renderer）
- MIDI JSON → `.mid` 写入（确定性）
- MIDI → SoundFont（FluidSynth）合成音频（本地闭合）
- 产物落盘与日志/可审阅资产归档
...
- **创意审稿闭环**：由 Ocelot 统一审稿（最多 3 轮），目标是“忠实表达”，不是“文学性更强”；审稿意见必须可执行、可收敛
