...
- [x] Sync docs vs code: `internalNotes` requiredness
- [x] Sync docs vs code: `RenderTransition.direction` (slide only)
- [x] Align Ocelot `outputSchema` with RenderScript contract (transition/kenBurns)
- [x] Ensure failures always write `error.log` (not only render failure)
- [x] Prevent duration rounding drift (seconds → frames)
...
- Severity: `Low`
- Status: `Resolved`
- Location: `.github/docs/LihuaCat 产品地基.md:304`
...
- Resolution evidence: Resolved in `d13cd45` + `pnpm test` PASS.
...
- Resolution evidence: Resolved in `d13cd45`.
...
- Severity: `Medium`
- Status: `Resolved`
- Location: `src/prompts/render-script.prompt.ts:18`
...
- Validation: `pnpm test`
- Resolution evidence: Resolved in `d13cd45` + new test `tests/validate-render-script.spec.ts` PASS.
...
- Resolution evidence: Resolved in `d13cd45` + new test `writes error.log when workflow fails before render stage` PASS.
...
- `d13cd45`: 文档与计划同步（`internalNotes` 必填、`RenderTransition.direction`）；对齐 Ocelot `outputSchema`；新增帧级语义校验；保证早失败写 `error.log`；修正测试命名。
